...
exports.run = async (client, message, args) => {
  ...
  if (!args[0]) {
    ...
    await db.set(`afkSebep_${message.author.id}_${message.guild.id}`,"Şuanlık AFK'yım En Kısa Sürede Size Cevap Vereceğim.");
    ...
    const ramo = await db.fetch(`afkSebep_${message.author.id}_${message.guild.id}`);
